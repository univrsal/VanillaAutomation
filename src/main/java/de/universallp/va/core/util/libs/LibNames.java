package de.universallp.va.core.util.libs;

/**
 * Created by universallp on 08.08.2016 17:51.
 * This file is part of VanillaAutomation which is licenced
 * under the MOZILLA PUBLIC LICENSE 1.1
 * github.com/univrsal/VanillaAutomation
 */
public class LibNames {
    public static final String MOD_ID = "va";
    public static final String MOD_NAME = "VanillaAutomation";
    public static final String MOD_VERSION = "1.12.2-1.5.3";
    public static final String CLIENT_PROXY = "de.universallp.va.client.ClientProxy";
    public static final String SERVER_PROXY = "de.universallp.va.core.CommonProxy";

    // Misc
    public static final String MOD_QUARK = "Quark";
    public static final String CRASHREPORT = "de.universallp.va";
    private static final String PREFIX = MOD_ID + ":";

    // Tiles
    public static final String TILE_PLACER  = PREFIX + "tilePlacer";
    public static final String TILE_XPHOPPER =  PREFIX +"tilexpHopper";
    public static final String TILE_FILTEREDHOPPER =  PREFIX +"tilefilteredhopper";
    public static final String TILE_CLOCK =  PREFIX +"tileredstoneclock";

    public static final String GUI_DIR = "va.dir.";
    public static final int TEXT_COLOR = 4210752;
    // Blocks
    public static final String BLOCK_PLACER = PREFIX + "blockplacer";
    public static final String BLOCK_XPHOPPER = PREFIX + "xphopper";
    public static final String BLOCK_FILTEREDHOPPER = PREFIX + "filteredhopper";
    public static final String BLOCK_CLOCK = PREFIX + "redstoneclock";

    // Items
    public static final String ITEM_GUIDE = PREFIX + "vaguide";
    public static final String ITEM_DESCRIPTIONTAG = PREFIX + "descriptiontag";

}
